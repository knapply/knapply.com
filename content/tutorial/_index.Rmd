...
build_link <- function(topic, slug, e = parent.frame()) {
  glue_html('<a href="/post/<~slug~>"><~topic~> </a>')
}
...
build_icon <- function(lang) {
  icon_label <- switch(lang, r = "R", py = "Python", rcpp = "Rcpp", cpp = "C++", numpy = "NumPy")
  # icon_label <- tooltip
...
toc_df <- tibble(
  ` ` = c(
    rep(build_link("Describing Data", "describing-data"), 3),
    rep(build_link("Central Tendency", "central-tendency"), 1)
    ),
  Topic = c(
    build_link("Length/Size of a Collection", "describing-data/#lengthsize"),
    build_link("Minimum and Maximum Values", "describing-data/#minimum-and-maximum-values"),
    build_link("Sum", "describing-data/#sum"),
    build_link("Mean", "central-tendency/#mean")
    ),
  r_icon = c(rep(check, 4)),
  rcpp = c(rep(check, 4)),
  cpp = c(rep(check, 4)),
  py = c(rep(check, 4)),
  numpy = c(rep(check, 4))
  ) %>% 
  set_names(c(" ", "Topic", build_icon("r"), build_icon("rcpp"), build_icon("cpp"), 
              build_icon("py"), build_icon("numpy")))

toc_df %>% 
  kable(escape = FALSE, align = "c") %>% 
  kable_styling(font_size = 16, bootstrap_options = c("responsive")) %>% 
  column_spec(1, color = "green", bold = TRUE, width = "20%", italic = TRUE, background = "#dee0e2",#) %>% 
